...
מודל 8 שלבים — source of truth: `lib/manychat/stages.ts` → `V2_PIPELINE_STAGES`.

| stage | מתי |
|---|---|
| (השאר ריק / null) | pre-quote — שאלון רץ, עוד לא נשלחה הצעה |
| `INITIAL_QUOTE_SENT` | הצעה ראשונית נשלחה, מחכים לתגובה ראשונה תוך ~24h |
| `AWAITING_FIRST_RESPONSE` | חלפו 24h+ ללא תגובה מאז ההצעה הראשונית |
| `SHOWED_INTEREST` | הלקוח ענה משהו שמראה עניין — שאל שאלה, ביקש המשך, ביקש התאמה |
| `FACTORY_CHECK` | צריך מפעל: לוגו/מפרט/מחיר. ה-notes מזכיר "שלחתי למפעל" / "ממתין למפעל" / "ספק". |
| `FINAL_QUOTE_SENT` | אלי שלח מחיר סופי (אחרי תשובת מפעל), מחכה לתגובה |
| `NEGOTIATING` | quote + מילים "יקר"/"הנחה"/"להוריד" / לקוח מתלבט / מבקש שיחה לסגור |
| `WON` | תשלום / "סגרנו" / "הזמנתי" |
| `LOST` | סירוב מפורש / 3+ פולואפים ללא תגובה / 60+ ימי שתיקה / "לא מעוניין" — **חובה לציין `loss_reason`** |

#### `loss_reason` (כש-stage=LOST)

חובה לבחור אחד מ:

- `יקר_לו` — מחיר היה גבוה מדי
- `לא_ענה` — שתיקה אחרי 3+ פולואפים
- `לא_רלוונטי` — לא התאים לעסק שלו
- `מצא_ספק_אחר` — בחר מתחרה
- `זמן_אספקה` — זמן האספקה לא מתאים
- `כמות` — כמות לא תואמת

### 3c. Flags (אפס או יותר)

קיימים: `דחוף`, `עסקה_גדולה`, `ביקש_שיחה`, `אחרי_החג`, `מועדף` (`מועדף` ידני בלבד — אל תציע).
חדשים (אופציונליים): `לקוח_חם` (קבע כשהשלב SHOWED_INTEREST), `לא_ענה` (אחרי פולואפ ללא תגובה), `מחכה_למפעל` (קבע כשהשלב FACTORY_CHECK).
...
| משוך 0 הודעות, אין notes, אין quote | השאר stage=null (pre-quote) אם נרשם לפני <7 ימים, אחרת בדוק `last_interaction` — אם 30+ ימי שתיקה → `LOST` + `loss_reason='לא_ענה'` |
