...
- `/api/factory/refresh` checks `CRON_SECRET` only; others accept `BOT_SECRET`/`CALL_TRIGGER_SECRET`. Trigger by hand with `CRON_SECRET`/`CALL_TRIGGER_SECRET` (`BOT_SECRET` isn't pullable).
