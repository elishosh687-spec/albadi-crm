/**
 * Reassign EVERYTHING owned by Elazar → Itay: pipeline opportunities, their
 ...
 *
 *   DATABASE_URL=... npx tsx scripts/_reassign-elazar-to-itay.ts        # AUDIT (read-only)
 *   DATABASE_URL=... npx tsx scripts/_reassign-elazar-to-itay.ts --go   # EXECUTE
 *
...
(async () => {
  ...
  const elazar = findUser("אלעזר") || findUser("Elazar") || findUser("Shoshtari");
  const itay = findUser("איתי") || findUser("Itay") || findUser("המלך");
  if (!elazar || !itay) {
    console.log("users found:", users.map((u) => `${fullName(u)} <${u.email}> ${u.id}`).join("\n"));
    throw new Error("could not resolve Elazar/Itay");
  }
  console.log(`FROM Elazar = ${fullName(elazar)} (${elazar.id})`);
  console.log(`TO   Itay   = ${fullName(itay)} (${itay.id})`);
  ...
  // 3) crm_tasks owned by Elazar (open ones matter most; count both).
  ...
  console.log("\n=== SCOPE (what will move Elazar → Itay) ===");
  console.log(`opportunities owned by Elazar : ${elazarOpps.length}`);
  console.log(`   → distinct contacts (leads): ${contactIds.length}`);
  console.log(`crm_tasks owned by Elazar     : ${taskRows.length} (open: ${openTasks.length})`);
  console.log(`total opps in all pipelines   : ${allOpps.length}`);
  if (elazarOpps.length) {
    console.log("\n=== FULL LIST — leads (opportunities) owned by Elazar ===");
    elazarOpps
      .slice()
