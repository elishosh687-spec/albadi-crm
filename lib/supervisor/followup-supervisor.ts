...
export interface FollowupSuperviseInput {
  ...
  /** Stage-keyed template label (MID_QUESTIONNAIRE / AWAITING_ESTIMATE / etc). */
...
- Stage is WAITING_FACTORY — text follow-up rarely right; Eli should call
...
Context: Eli notes = "אמר ליצור איתו קשר ב-18.5.26". Today is 2026-05-18. Stage = AWAITING_ESTIMATE. attempt = 1.
→ override_with_text:
  reason: "אסף ביקש קשר היום ב-18.5 — מזכיר ספציפית"
  override_text: "אסף, היום ה-18.5 כמו שביקשת. אפשר להתקשר עכשיו?"

Context: Generic lead, no last inbound, sat 24h after quote. Stage = AWAITING_ESTIMATE. attempt = 1.
→ approve_template:
  reason: "אין קונטקסט מיוחד — טמפלייט תקין"

Context: Lead stage = WAITING_FACTORY. Eli notes empty. attempt = 1.
→ escalate_to_eli:
  reason: "WAITING_FACTORY — אלי צריך להתקשר, לא טקסט"
