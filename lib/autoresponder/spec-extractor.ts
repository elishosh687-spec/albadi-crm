...
- custom = כל מספר אחר (500, 1500, 7500, 2000).
  IMPORTANT: quantityCustom חייב להיות מחרוזת ספרתית בלבד של המספר הקנוני —
  ספרות בלבד, ללא יחידות, ללא ניקוד, ללא מילים. דוגמאות:
    "אלפיים" → "2000"
    "חמש מאות" → "500"
    "שבעת אלפים וחמש מאות" → "7500"
    "1500 יחידות" → "1500"
    "1,500" → "1500"
  זה מאפשר ל-engine להמיר ל-tier אוטומטית.

מיפויים — product (מידה):
פורמט מפעל אחיד: "H{גובה}*D{עומק}*W{רוחב}" בס״מ.
- p1 = H25*D8*W20 (קוסמטיקה, תכשיטים, קטן)
- p2 = H30*D10*W30 (ביגוד קל, מתנות, בינוני)
- p3 = H30*D12*W40 (נעליים, ביגוד)
- p4 = H50*D15*W40 (פריטים גדולים)
- p5 = H40*W30 (שטוחה, פריטים רחבים — אין עומק)
- p6 = H15*W20 (קטנה, פריטים קטנים — אין עומק)
- custom = מידות לא סטנדרטיות.

IMPORTANT — productCustom MUST be in the factory's canonical format:
  "H{height}*D{depth}*W{width}" (מספרים בלבד, בס״מ, ללא יחידות).
  אם שטוחה ללא עומק: "H{height}*W{width}".

  המר כל ניסוח של הלקוח לפורמט הזה:
    "30 גובה 20 רוחב 10 עומק" → "H30*D10*W20"
    "30×40×20" + הלקוח לא ציין סדר → נסה לפי הקשר; אם לא בטוח,
      השאר את הטקסט המקורי ב-productCustom והוסף ב-notes:
      "סדר המספרים לא ברור — לקוח כתב: 30×40×20".
    "H50*D15*W40" → "H50*D15*W40" (כבר בפורמט).
    "30 על 40" (שני מספרים בלבד) → "H40*W30" (גובה אחרון, רוחב ראשון —
      ניסוח עברי טיפוסי) — או הוסף הבהרה ב-notes אם מעורפל.
    "20×8×25" → "H25*D8*W20" (W×D×H ניסוח ישן → המר ל-H*D*W).
