...
## 14. מחשבון משוער (self-quote estimator) + מודל אריזה (חדש, 2026-06-24)

לשונית **"מחשבון משוער ✨"** ב-[`CalculatorView`](../components/calculator/CalculatorView.tsx) (widget + v3, operator-only). מתמחרת שקית 80g **שרירותית** (כל H·D·W, ידיות, למינציה, צבעים, כמות ≤10,000) **בלי לשאול את המפעל** — בוחרת את המפעל הזול שמייצר, מנמקת, ומסמנת ביטחון. מעל 10,000 → "שלח למפעל".

### 14.1 מחיר מפעל — מודל פר-מפעל
[`lib/factory/estimator.ts`](../lib/factory/estimator.ts) `estimateFactoryCny` → מקדמים ב-`app_config` key `factory_estimators` ([`estimator-config.ts`](../lib/factory/estimator-config.ts)), פר מפעל פר tier (3000/5000/10000), `area = 2HW+2HD+WD`:
`base(area)=makeFee+perCm2·area` (1-צבע) + add-ons צבעים/ידיות/למינציה; 版费 כשורה חד-פעמית נפרדת. **למינציה ⇒ Mandy בלבד** (亚森 מלמן בתפירה → מפעל). מאומת LOO חציון ~4–7%. תוצאה → `priceFactoryQuote` (אותו מנוע מרווח/שילוח). מקור: [`server/estimator-fit.ts`](../lib/factory/server/estimator-fit.ts), generator [`scripts/fit-estimator.ts`](../scripts/fit-estimator.ts).

### 14.2 מודל אריזה / CBM (VERIFIED 2026-06-24, workflow wf_699152fc-834)
מחליף את `nearestCarton` (ששאל את הקרטון של המוצר הקרוב) ב-`predictCarton`. הפיזיקה: שקית מקופלת נערמת ב-`area × עובי`, לכן:
```
CBM ליחידה (m³) = T_mm · area · 1e-7      area = 2HW+2HD+WD (cm²)
T = 0.85 mm (פר-מפעל: 亚森 0.78 · Mandy 0.84 · 鼎驰 0.83)
יח׳/קרטון = round_to_25( 0.85 · נפח-קרטון-סטנדרטי(60×40×40) / נפח-מקופל )
```
`area` ניצח כל מאפיין אחר ב-LOO. בחירת הקרטון הסטנדרטי **מתבטלת** מ-CBM/יח׳ (ארגז גדול ⇒ יותר יח׳). מאומת מול 33 הצעות אמיתיות + קטלוג: **שקיות עם דופן (D>2) — LOO חציון ~5.5–6.8%, מקס ~12% ✅**.

- **שערי ביטחון (`confidence:'low'` → תג כתום "אומדן — לאמת מול מפעל"):** חומר≠80g non-woven, או שטוח (D≤2), או מגש (H<10), או שטח∉[1500,5400]. בצורות האלה העובי מתפזר 0.52–1.6 מ״מ ואי אפשר לחזות (LOO עד ~60%) — מסמנים, לא מנחשים. תת-תמחור שם הוא הכיוון היקר.
- **משקל לא נחזה מהמידות** (LOO ~19%): המשקל הרשום הוא קרטון **גרוס** (≈1.67× בד). מוצג אומדן-בד גס לאוויר בלבד; ים (~90% מההזמנות) מתומחר ב-CBM. אם יש נתון מפעל אמיתי — קוראים `weightKg/cartonQty`, לא חוזים.
- **אינטגרציה:** `predictCarton` מחזיר `CartonEstimate` באותו מבנה → המנוע (`engine.ts`/`pricing.ts`) ללא שינוי; `totalCbm` = `CBM/יח׳ × כמות` (סטייה 0.2%).

### 14.3 שיפור אוטומטי (refit יומי)
[`server/refit-estimator.ts`](../lib/factory/server/refit-estimator.ts) (cron 04:00, `/api/factory/refit-estimator`) קורא קטלוג + quote-log + הצעות `factory_quote_requests` חדשות, מסנן ל-80g non-woven, מדדפ, משמיט שטוח/מגש, מסגיר שורות חשודות (`FIRBM6CX`), מתאים מחדש גם מחיר וגם אריזה, ומפרסם רק אם LOO ≤ הסף; אחרת שומר ישן + DM לאלי.

### 14.4 דיוק נמדד מול הצעות אמיתיות (בטווח 3000–10000, המינימום החדש)
מחיר מפעל ~7% חציון · CBM ~7% חציון. צורות לא-תקניות מסומנות לאימות. (הזמנות <3000 לא רלוונטיות — מינימום ההזמנה אצלנו 3000.)
