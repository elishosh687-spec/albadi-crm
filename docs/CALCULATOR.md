...
יש **שני מחשבונים נפרדים** במערכת — אותו רעיון, מתמטיקה שונה:

| מסך | מנוע | קונבנציית רווח | מתי משתמשים |
|---|---|---|---|
| **Calculator** (Dashboard + Widget) — `/dashboard/v3/calculator` + `/widget/calculator` | `lib/factory/calculator/engine.ts::calculateQuote` | **Margin-on-price** (רווח ÷ מחיר מכירה) | חישוב מקדים — לפני שיש Quote רשמי מהמפעל |
| **FinalizeModal** ("חישוב הצעה סופית") | `lib/factory/pricing.ts::priceFactoryQuote` | **Markup-on-cost** (רווח ÷ עלות) | חישוב סופי — כשמתקבל מחיר מהמפעל ולפני שולחים PDF ללקוח |

> ⚠️ **חשוב להבין:** 50% margin-on-price ≠ 50% markup-on-cost.
> Margin 50% על מחיר ₪10 = ₪5 רווח על ₪5 עלות (markup 100%).
> Markup 50% על עלות ₪10 = ₪5 רווח על ₪15 מחיר (margin 33%).
> שני המסכים אצלך משתמשים בקונבנציות שונות. מומלץ באיזשהו שלב לאחד.
...
## 3. FinalizeModal (`priceFactoryQuote`) — Markup-on-Cost
...
### 3.2 הנוסחה (markup-on-cost)

```
unitSellingPrice = unitCost × (1 + marginPct / 100) + unitShipping
unitProfit       = unitCost × (marginPct / 100)
```

**מה זה אומר:** ה-margin פה זה **אחוז הרווח מתוך העלות**, לא מתוך המחיר. כשאלי גורר את הסליידר ל-60% — זה אומר רווח של 60% על העלות.

**דוגמה:** עלות יחידה ₪6, margin 60%:
- selling = 6 × 1.6 = ₪9.60 (+ shipping)
- profit = 6 × 0.6 = ₪3.60
- = 37.5% מ-₪9.60 (margin-on-price equivalent)

### 3.3 שילוח — גם כאן pass-through

```
selling = cost × (1 + margin) + shipping            ← זהה לקונבנציה אחרת
profit  = cost × margin                              ← אין רווח על שילוח
```
...
```
marginPct = ((perUnit − unitShipping) / unitCost − 1) × 100   ← markup-on-cost
```
...
- [ ] **לאחד את הקונבנציות.** margin-on-price ב-Calculator vs markup-on-cost ב-FinalizeModal זה מבלבל. הצעה: לקרוא לסליידר ב-FinalizeModal "% רווח על העלות" במפורש, ובמחשבון "% רווח על המחיר", או לעבור לקונבנציה אחת בשני המקומות (margin-on-price מבחינה עסקית הוא הסטנדרט).
