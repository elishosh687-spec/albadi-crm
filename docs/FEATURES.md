...
| 1.2.6 | Custom-spec → FACTORY_WAIT (subFlow=awaiting_factory_estimate, אלי מתמחר ידני) | shipped | `lib/autoresponder/questionnaire.ts` | CUSTOMER-FLOW §2 |
...
### 1.4 Decision Flow (INTAKE / FACTORY_WAIT[awaiting_logo])

| # | Feature | Status | קוד | מסמך |
|---|---|---|---|---|
| 1.4.1 | INTAKE sub-flows (accept/negotiate/reject) | shipped | `lib/autoresponder/decision.ts` | CUSTOMER-FLOW §2 |
| 1.4.2 | Canned answers for FAQ (delivery, payment, format, meeting) | shipped | `lib/autoresponder/decision.ts` | BOT-COPY |
| 1.4.3 | Logo request (FACTORY_WAIT/awaiting_logo) on accept | shipped | `lib/autoresponder/decision.ts` | CUSTOMER-FLOW §3 |
| 1.4.4 | Logo received (image) → FACTORY_WAIT/awaiting_factory_estimate + NEEDS_ELI | shipped | `lib/autoresponder/decision.ts` | CUSTOMER-FLOW §3 |
...
| 1.8.2 | **GHL tag "start over" → reset questionnaire**. Eli drags the tag (aliases: `start over`, `התחל מחדש`, `שלח שאלון`, ...) onto a contact. App webhook resets `qState` to step 1 + `pipeline_stage = NULL`, sends transition + OPENING + first poll, strips the tag from GHL/DB so re-adding re-triggers. Dedupe via `bridge_events.evtId`; 60 s cooldown via `lastFollowUpAt`. Prior business data (factory draft, quote totals, summary, follow-up date, loss reason) is preserved on purpose so a re-quote keeps history. | shipped (2026-05-24) | `app/api/ghl/app-webhook/route.ts`, `app/api/integrations/inbound/ghl-tag/route.ts`, `lib/autoresponder/questionnaire.ts:restartQuestionnaire` | CHANGELOG 2026-05-24 |
| 1.8.3 | **NO_RESPONSE_REENGAGE stage + LLM re-engagement loop**. Manual stage Eli drags leads into after 3 calls + 3 messages without a reply. `app/api/bot/followups/route.ts` STAGE_RULE: cadence 3 days, `unbounded: true` so MAX_FOLLOWUPS cap is bypassed. Body per send built by `buildReEngagementMessage(sid)` — GPT-4o-mini reads name + notes + bot_summary + last 20 messages, returns 1-2 sentence Hebrew nudge in Eli's voice with the standard opt-out footer. Inbound at this stage is intercepted before the supervisor: `handleReengagementInbound` runs `classifyReengagementReply` (intent: `interest | removal | ambiguous`), pauses the bot, DMs Eli with the verdict + recommendation, leaves stage unchanged for Eli to move. Stop-word opt-out is a separate path that auto-moves to LOST. | shipped (2026-05-24) | `lib/autoresponder/re-engagement.ts`, `app/api/bot/followups/route.ts`, `lib/messaging/templates.ts` (RE_ENGAGEMENT FollowupStage + extended stop-word list) | CHANGELOG 2026-05-24 |
...
|---|---|---|---|---|
| 2.7.0 | **Hub widget** — שלל ה-widgets מאוחדים תחת Custom Menu Link אחד. ה-tab בר בחירה (query `tab=…`) ומציג iframe מתאים. Tabs: `📥 שיחות` (GHL inbox redirect), `🤖 בוט` (bot decisions), `💰 הצעות מחיר` (Quotes History — 2.7.8), `🧮 מחשבון`. הרצועה responsive: scroll אופקי, tap-targets 44 px, sticky על top. עדיף על menu links נפרדים פר-tab כי שומר state, מהיר יותר, פחות clutter. | shipped (2026-05-23) | `app/widget/hub/page.tsx` | `/widget/hub?widget_token=<T>&tab=<inbox\|bot\|factory\|calc>&sid=<contact-sid>` |
| 2.7.1 | 🧮 מחשבון מחיר — iframe sidebar. Wrapper `CalculatorWithSettings` עוטף את `CalculatorView` עם טאב הגדרות (FX, margin matrix, shipping rates). מצב standalone (אין `sid` ב-URL) או lead-aware (sid → preload בשם ה-lead בכותרת + preselect ב-QuoteShareCard). | shipped (2026-05) | `app/widget/calculator/page.tsx`, `components/calculator/CalculatorView.tsx`, `components/calculator/CalculatorWithSettings.tsx` | `/widget/calculator?widget_token=<T>&sid=<sid>` |
| 2.7.1a | **Calculator → WhatsApp send + lead picker.** אחרי חישוב, הפאנל `QuoteShareCard` מציג textarea של גוף הצעה (בעברית, אותו פורמט של הבוט), כפתור `העתק טקסט`, כפתור `שלח בווצאפ`. ה-picker (typeahead debounced 200 ms על `/api/widget/leads/recent`) מאפשר לבחור כל ליד מתוך הוידג'ט בלי לפתוח מחדש מתוך כרטיס לקוח אחר. כש-`sid` מגיע מ-URL → מוצג כ-chip ניתן להחלפה. | shipped (2026-05-24) | `components/calculator/CalculatorView.tsx:QuoteShareCard`, `app/api/widget/calculator/send-text/route.ts` | — |
| 2.7.2 | 📋 סיכום הזמנה (read-only) — iframe sidebar | deprecated (2026-05-23) | `app/widget/order-summary/page.tsx`, `components/order-summary/OrderSummaryView.tsx` | `/widget/order-summary?widget_token=<T>` |
| 2.7.3 | 🏭 הצעות מפעל (factory-flow) — flow מלא: בחר ליד → Order Summary → שלח Feishu → Refresh → Finalize (margin slider + shipping + DetailedBreakdown ¥→$→₪) → שלח WhatsApp עם PDF. עכשיו מאוחד עם Quotes History (2.7.8) ל-UI אחד תחת tab `💰 הצעות מחיר`. | shipped (2026-05-21) | `app/widget/factory-flow/page.tsx`, `components/factory-flow/*.widget.tsx`, `lib/factory/server/{list,refresh,finalize,sendWhatsapp}.ts`, `app/api/widget/factory/*` | `/widget/factory-flow?widget_token=<T>` |
...
| 2.7.6 | ↩️ **GHL → DB stage sync** (narrow webhook) — opportunity stage change in GHL UI doesn't fire OpportunityCreate; this narrow webhook handles `stage_changed` specifically. Mostly redundant with the full resync (2.7.7) + native app webhook (2.7.9) but kept for low-latency single-field updates. | shipped (2026-05-21) | `app/api/ghl/stage-changed/route.ts` | GHL Workflow → POST `/api/ghl/stage-changed` |
| 2.7.7 | ↩️ **GHL → DB full resync** — webhook catch-all שעל כל שינוי ב-Contact/Opportunity ב-GHL מושך contact + notes + tasks + opportunities ועושה merge ל-DB (leads, lead_tags, crm_tasks, opportunities). | shipped (2026-05-22) | `app/api/ghl/resync/route.ts`, `lib/ghl/resync-helper.ts:resyncContact` | GHL Workflow → POST `/api/ghl/resync` |
| 2.7.8 | **Quotes History widget** — רשימה קומפקטית של `factory_quote_requests` (ENTIRE pipeline, לא רק ליד נוכחי). 4 filter pills (all/pending/received/finalized) + חיפוש. כל שורה עם action buttons: 👁️ פתח modal (שמשתמש ב-`QuoteHtmlPreview` עם widgetToken), 📥 PDF, 💬 שלח WhatsApp, 🗑️ delete, 🔗 פתח GHL contact, 🧮 חשב (רק ל-`received` בלי `finalPricing`). Modal תומך גם בתצוגת לקוח וגם בתצוגה פנימית (בוס) עם DetailedBreakdown מלא. | shipped (2026-05-23) | `components/factory-flow/QuotesHistoryView.tsx`, `app/api/widget/quotes/list/route.ts`, `app/api/factory/[id]` (DELETE + widget_token support) | — |
| 2.7.9 | ↩️ **GHL Marketplace App native webhook** — `/api/ghl/app-webhook` מקבל events ישירות מה-App של GHL (לא דרך Workflow). מטפל ב-`Contact*` / `Opportunity*` / `Note*` / `Task*` / `ContactTagUpdate`. HMAC-SHA256 verify על `GHL_APP_WEBHOOK_SECRET`. Dedupe ב-`bridge_events.evtId`. כולל ה-branch של ה-"start over" tag (ראה 1.8.2). | shipped (2026-05-23) | `app/api/ghl/app-webhook/route.ts` | GHL Marketplace App → webhook URL `/api/ghl/app-webhook` |
