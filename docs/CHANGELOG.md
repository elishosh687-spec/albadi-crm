...
- **שליחה מהמחשבון המשוער.** מהלשונית אפשר עכשיו: (1) **לשלוח אומדן ראשוני ללקוח** — PDF מסומן "אומדן מחיר ראשוני" (כפוף לאישור מפעל, תוקף 7 יום) + הודעת WhatsApp, דרך `sendEstimateToCustomer` (estimateFactoryCny→priceFactoryQuote→renderCustomerQuotePdf→Blob→sendBridgeMessage, ללא שורת DB). (2) **לבקש הצעת מחיר מהמפעל** לפי המפרט — שימוש חוזר ב-`createFactoryRequest` (יוצר שורת `factory_quote_requests` + Feishu, **בלי לשנות את שלב הליד**); ההצעה האמיתית חוזרת דרך זרימת ה-finalize הרגילה. בורר-ליד אחד משותף ל-3 הפעולות ב-`QuoteShareCard`.
- **למינציה לפי צבעים — שקיפות.** הצבעים כבר מתומחרים בלמינציה דרך ה-版费 (ליניארי per-color); הוספת שורת הסבר per-unit מפורשת ("N צבעים × ¥X/צבע = ¥Y חד-פעמי ≈ ¥Z/יח׳") כך ש-3 צבעים ≠ 5 צבעים גלוי. נוסף `lamColor` (אופציונלי) + hook התאמה שיופעל אוטומטית אם יגיעו הצעות למינציה רב-צבעוניות (אין כיום → per-unit נשאר 0, ללא המצאה).
...
- **עריכת כל פרטי ה-PDF ב-FinalizeModal.** כרטיס המוצר הפך מקריאה-בלבד לשדות עריכה — שם מוצר (כותרת ה-PDF), מידות (W/H/D), כמות (מתמחר מחדש חי), חומר, הדפסה, גימור, והערות ללקוח. מספר ההצעה מוצג בכותרת החלון. ה-spec המעודכן נשמר ל-`productSpec` בעת finalize. דרך `specOverride` ב-finalize routes. דאשבורד + widget.
- **תמונת מוצר ב-PDF.** התמונות מוטמעות בתא ב-Feishu (fileToken + לינק auth-gated). `lib/feishu/media.ts` מוריד אותן ב-tenant token ומעלה ל-Blob ציבורי. במסך: כפתור **"העלה תמונה"** (קובץ → Blob, `POST /api/factory/upload-image`) ו**משיכה אוטומטית מ-Feishu** בפתיחת החלון (`POST /api/factory/[id]/pull-image`). מוטמעת ב-PDF (בודד + משולב) דרך `fetchImageDataUri`.
- **איחוד כמה הצעות ל-PDF אחד.** בחירה-מרובה (לכל לקוח) בשתי רשימות ההיסטוריה + פאנל inline בלשונית "שיחות" (inbox). פריסת **חשבונית** — שורה לכל מוצר + סה"כ כולל, עם תמונה קטנה. קישור ציבורי + שליחת WhatsApp. `GET /api/factory/combine/pdf?ids=...`.
- **תמחור משלוח משולב.** מוצרים שנשלחים יחד → השילוח מחושב **פעם אחת** על ה-CBM/משקל המאוחד (רצפת 1-CBM נספרת פעם אחת → זול יותר) ומקופל בחזרה למחיר הלקוח. **הרווח לא משתנה** (שילוח = pass-through). פאנל **"חישוב משולב"** ב-FinalizeModal עם **סליידר מרווח לכל מוצר** + סליידר "קבע לכולן" + סיכום משולב חי; גם ה-PDF המשולב משתמש בשילוח הזול. `lib/factory/combined.ts`. כולל הצעות במצב `received` (מתומחרות לפי מרווח ברירת-מחדל).
- **ייבוא מחדש של הצעות שנמחקו מ-Feishu.** "ייבא מ-Feishu" סורק את הגיליון ויוצר מחדש הצעות שמספר ההצעה שלהן נעלם מה-DB — שומר את **אותו מספר הצעה**, ה-spec, תשובת המפעל, והתמונה. ליד מותאם לפי **שם מנורמל** (חסין גרש/אפוסטרוף/סימני RTL); שורות שלא הותאמו מקבלות **בורר ליד ידני**. `lib/factory/server/import-from-feishu.ts` + `POST /api/factory/import-feishu[/assign]`.

### Changed

- **הצעה סופית אוחדה ל-margin-on-price** (היה markup-on-cost): 40% = 40% מהמחיר בכל מקום, סליידר 0–99%. הנוסחה ההפוכה ("תמחור לפי יעד") הוצאה ל-`marginPctFromUnitPrice` משותף. ראה `CALCULATOR.md`.
- **התאמת מספר הצעה ב-Feishu סובלנית לסיומת "-A"** (revision): `baseQuoteNo` משווה לפי המספר הבסיסי — מתקן זיהוי בייבוא **וגם** את התאמת השורה ברענון.
- ה-PDF הסופי נשמר ב-Blob תחת **path טרי בכל הפקה** (`addRandomSuffix`) — כדי שעדכון תמונה/מחיר לא יוגש מגרסת cache ישנה.

### Fixed

- **הרענון כבר לא דורס את `productSpec` מ-Feishu.** הסנכרון משך גם את עמודות המוצר (A..J), וכשהשורה לא מיושרת (drift / quotationNo חסר) זה כתב נתונים של הצעה אחרת ("מחירים/הצעות לא קשורים"). עכשיו רק תשובת המפעל מסונכרנת; פרטי המוצר נערכים ידנית בחלון.

### Why

מחזור עבודה אמיתי של אלי: לקוח מזמין כמה מוצרים שנשלחים במשלוח אחד. הוא צריך (א) לערוך מה שיוצא ב-PDF, (ב) תמונה אמיתית ב-PDF, (ג) לראות את הרווח/המרווח **על כל ההזמנה יחד** עם שילוח מאוחד זול, ו-(ד) להחזיר הצעות שמחק. כל אלה נבנו סביב שני מנועי התמחור הקיימים בלי לשנות את הליבה.
...
- **NO_RESPONSE_REENGAGE pipeline stage + LLM re-engagement loop.** New manual stage Eli drags leads into after 3 calls + 3 messages without a reply. The followup cron then nudges every 3 days (skipping quiet hours + Sabbath/holiday) with a body written per-send by GPT-4o-mini based on the lead's name + `notes` + `bot_summary` + last 20 messages. Loop is `unbounded` (no MAX_FOLLOWUPS cap) — only stops on customer reply (auto-pause + Eli DM with intent classification), stop-word opt-out (auto-move to LOST + DM), or manual drag. Footer on every send: "אם אינך מעוניין/ת לקבל הודעות נוספות, השב/י 'הסר'…". Files: `lib/autoresponder/re-engagement.ts`, `app/api/bot/followups/route.ts`, `lib/messaging/templates.ts` (RE_ENGAGEMENT FollowupStage + stop-word list extended). Inbound classifier branch wired into both `greenapi/webhook` and `bridge/webhook` so the bot pauses + DMs Eli with intent (`interest | removal | ambiguous`) + reason + recommendation as soon as the customer engages.
- **FUTURE_FOLLOW_UP pipeline stage** mapped in `GHL_STAGE_IDS`. Eli had this stage in the GHL pipeline since day one but it wasn't in the env map, so dragging an opp there caused `reverseLookupStage` to return null → DB stays at the prior stage → next `syncLeadToGHL` push reverts the opp in GHL. Adding the env var closes that loop. Same risk exists for any NEW stage added in GHL UI without an env entry — see "Known gap" below. File: `integrations/ghl/config.ts`.
- **"Start over" tag flow.** GHL App webhook `ContactTagUpdate` now recognizes the tag aliases (`start over`, `start_over`, `startover`, `התחל מחדש`, `שלח שאלון`, `שאלון מחדש`, `restart_questionnaire`, `restart bot`). When added, the handler:
  1. Dedupes via `bridge_events.evtId` (`onConflictDoNothing + returning`) so GHL retries don't double-fire.
  2. Cooldown: if `lastFollowUpAt < 60s ago`, skip but still try to strip the tag so it doesn't get stranded.
  3. Removes the tag from GHL + local mirror BEFORE the slow restart so any in-flight retry sees an empty tags array.
  4. Calls `restartQuestionnaire(sid, transitionText)` — minimal reset (qState back to step 1, `pipeline_stage = NULL`, `followUpCount = 0`, `botPaused = false`, `pipelineFlag = null`). Prior business state (`factorySpecDraft`, `quoteTotal`, `botSummary`, `lossReason`, etc) is preserved on purpose so a re-quote keeps history.
  Files: `app/api/ghl/app-webhook/route.ts`, `app/api/integrations/inbound/ghl-tag/route.ts` (legacy workflow path mirrors the same behavior), `lib/autoresponder/questionnaire.ts`.
- **Calculator widget → WhatsApp send.** New panel in `CalculatorView` (the `/widget/calculator` iframe) below the breakdown: read-only textarea with a Hebrew quote body matching the bot's format, `העתק טקסט` (clipboard), and `שלח בווצאפ` (resolves the lead → JID, POSTs to new `/api/widget/calculator/send-text`, fires `sendBridgeMessage` with `sender='eli'` → also mirrors to GHL Inbox). Lead picker (typeahead → `/api/widget/leads/recent`) lets Eli pick any lead from inside the widget without reopening from a specific contact card. URL param `sid` preselects when set.
- **Hub widget shell** unified all per-contact iframes (`📥 שיחות`, `🤖 בוט`, `💰 הצעות מחיר`, `🧮 מחשבון`) into one Custom Menu Link. Tabs are mobile-friendly: horizontal scroll, 44 px tap targets, sticky header, no flexWrap. Removed the old `📋 הזמנה` tab (redundant with Quotes + Calculator). Files: `app/widget/hub/page.tsx`.
- **Quotes History widget** (the new `💰 הצעות מחיר` tab) — compact row list of `factory_quote_requests`, 4 filter pills (all/pending/received/finalized) + search, action buttons per row: 👁️ view modal, 📥 PDF, 💬 WhatsApp send, 🗑️ delete, 🔗 GHL contact, 🧮 calculate (for unpriced `received` rows). Modal renders the existing `QuoteHtmlPreview` with `widgetToken` prop so the boss tab can hit `/api/factory/config?widget_token=…`. Files: `components/factory-flow/QuotesHistoryView.tsx`, `app/api/widget/quotes/list/route.ts`.
...
- **GHL Inbox mirrors moved off the inbound critical path.** The two `forwardMessage` calls per reply (inbound + outbound mirror) were each blocking 200-500 ms while the customer waited for the bot's next question. Wrapped in Next 16 `after()` (from `next/server`) so the lambda returns the HTTP response immediately and the mirror keeps running in the background. Failure stays audited in `bridge_events` via `forwardMessage`'s internal `auditMirror`. CLI scripts (`tsx`) fall back to inline `await` so they don't silently drop mirrors. Files: `lib/bridge/client.ts` (outbound mirror inside `sendBridgeMessage`), `app/api/greenapi/webhook/route.ts` (inbound + eli-manual outbound mirror).
- **Spec-extractor LLM fallback timeout** dropped from 7 s → 1.5 s for the single-field path (`extractSingleField`). Added a `timeoutMs` param to `extractSpecFromText`; step 9 free-text revisions keep the 7 s default because they legitimately need to extract multiple fields at once. Reasoning: on a poll-vote where `matchAnswer` already failed, a slow LLM answer just makes the customer wait — the reask path is fast and recoverable. File: `lib/autoresponder/spec-extractor.ts`.
- **`saveState` + outbound send in parallel** at the three hottest paths in `handleInbound` (normal advance, custom-text captured, size-page-2 pivot). `Promise.all([saveState, askQuestion])` shaves ~10-50 ms per reply with zero risk: the qState write and the bridge send don't depend on each other and the function still awaits both before returning. File: `lib/autoresponder/questionnaire.ts`.
- **Supervisor LLM hard-bypass during questionnaire.** `dispatchSupervisor` returns `{ shouldRunLegacy: true }` immediately when `qState.step <= 9 && !doneAt && !bailed`. Poll matching is deterministic — there's no useful judgment the LLM can add — and skipping saves the 1-3 s LLM round-trip per poll-vote reply. File: `lib/supervisor/server/dispatch.ts`.

### Fixed

- **Feishu factory sheet — column-index off-by-one.** The supplier-side sheet auto-fills column C (date) via a formula. Our outgoing 9-cell `buildFactoryRow` skipped that column on append, so the supplier's response columns landed at K..R instead of J..R. Result: Galit's row was parsed as `unitCostCny=2000` (was actually her quantity), `supplier="10"` (was her weight) and the calculator computed a ₪54M total. Fix: index map shifted +1, plus `buildFactoryRow` now writes the date in C explicitly (as a literal "YYYY-MM-DD" string instead of an Excel serial that rendered as "46164" until the supplier set the column type to date). `scripts/reparse-factory-responses.ts` is a one-shot that re-reads stored `feishuRowIndex` rows and rewrites `factoryResponse` with the correct parse — already run against prod. Files: `lib/feishu/sheets.ts`, `scripts/reparse-factory-responses.ts`.
- **Factory PDF attachment didn't render in GHL Inbox.** `forwardMessage` builds the GHL upload URL by sniffing the mime type, and the factory-send path passes a mediaUrl ending in `/pdf` but no mime. `extFromMime(undefined)` returned `"bin"`, so the proxy URL ended `.bin` and GHL's whitelist silently rejected the attachment — the bridge WA send succeeded but the GHL thread showed no file. Added `sniffMimeFromUrl(url)` as a fallback (URL path ends in `.pdf` / `/pdf` → `application/pdf`, same for image/audio/video). `sendBridgeMessage` also now forwards `mediaFilename` to `forwardMessage` so the GHL attachment carries a real filename (e.g. `הצעת-מחיר-RLSGZ6BH.pdf`) instead of `file.pdf`. Files: `integrations/ghl/sync.ts`, `lib/bridge/client.ts`.
- **`/api/factory/[id]` DELETE rejected widget callers.** Handler did its own cookie check and ignored the `widget_token` query param, so the Quotes History widget's delete button returned 401. Added widget-token fallback alongside the cookie check (middleware already gates the path). File: `app/api/factory/[id]/route.ts`.
- **FinalizeModal margin defaulted to 40 % instead of the per-quantity matrix.** On a fresh finalize (no prior `finalPricing`), `setMargin` is now seeded from `cfg.profitMarginByQuantity` with snap-down to the nearest tier; falls back to `cfg.defaultProfitMargin` only when the matrix is empty. Same fix applied to both the dashboard and widget variants. Files: `app/dashboard/v3/_components/factory/FinalizeModal.tsx`, `components/factory-flow/FinalizeModal.widget.tsx`.
- **Sea-vs-air comparison missing from the boss view** in `QuoteHtmlPreview`. The breakdown helper already supported an `alt` prop; the dashboard calculator path passed it but the factory-quote modal didn't. Now computed on the fly inside `QuoteHtmlPreview` from the same shipping options config (`cfg.shippingOptions` opposite-type lookup, engine's shipping formula). File: `app/dashboard/v3/_components/factory/QuoteHtmlPreview.tsx`.
- **חשב button + margin slider polish** in the Quotes History widget — for `received` quotes without `finalPricing`, the row now shows a one-click "calculate" entry point into `FinalizeModalWidget` pre-filled with the factory response. Styling normalized: icon-only button matching siblings instead of the wider bordered "חשב" label that was breaking row alignment. File: `components/factory-flow/QuotesHistoryView.tsx`.

### Operational notes

- New env vars (Production + Development; Preview was skipped because Vercel requires a non-main git branch for preview envs and we don't use any):
  - `GHL_STAGE_FUTURE_FOLLOW_UP = 6cfe4daa-ef2e-46fb-93a2-1236293ce4a6`
  - `GHL_STAGE_NO_RESPONSE_REENGAGE = a4363dca-2925-4877-8911-de4946145425`
- Vercel cron unchanged (`0 9 * * *` UTC = 12:00 Asia/Jerusalem) hitting `/api/bot/followups`. The new NO_RESPONSE_REENGAGE rule's cadence is 3 days from `lastFollowUpAt`, no cap.
- Legacy ManyChat-template scripts (`scripts/restart-send.ts` etc) remain in the repo for reference but are not in use — bridge has no 24h window so free-form re-engagement replaces template fallbacks.
- Vercel was redeployed via empty commit (`7e0a49f`) after env vars were set so the running runtime picked them up.

### Known gap

- Pipeline-stage drift detection. `reverseLookupStage` returning null on an unmapped GHL stage causes the "drag reverts overnight" bug we just fixed for FUTURE_FOLLOW_UP. If anyone adds a new stage in the GHL UI without adding the matching env var, the bug returns silently. Long-term fix would be either (a) a startup self-check that fetches the live pipeline and warns when a stage is unmapped, or (b) move the stage map into a DB table that the resync helper keeps in sync. Today it's a manual `npx tsx scripts/_diag-ghl-stages.ts` style diag (one-shot) when the pipeline shape changes.
...
- **GHL delivery loop eliminated — no more duplicate customer messages.** `postOutboundMessage` posts to `/conversations/messages`, which makes GHL fire the Custom Provider deliveryUrl webhook to actually deliver via the provider. Our deliveryUrl `/api/integrations/outbound` was then re-sending the same text via Green API, so every outbound mirror caused the customer to receive a duplicate. Fix: a 60-second text+contact dedup in `/api/integrations/outbound` blocks the loop — when GHL fires the callback after our mirror, the dedup sees the outbound row our handler just wrote (sender='bot' or 'eli') and returns 200 OK without re-sending. A previous attempt to switch the endpoint to `/conversations/messages/inbound` with `direction: "outbound"` stored the message correctly but the Inbox UI did not render `source: api` outbound bubbles coming from the inbound path — reverted to the original endpoint. ([integrations/ghl/client.ts](integrations/ghl/client.ts), [app/api/integrations/outbound/route.ts](app/api/integrations/outbound/route.ts))
