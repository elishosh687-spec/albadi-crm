...
- **GHL delivery loop eliminated — no more duplicate customer messages.** `postOutboundMessage` was POSTing to `/conversations/messages`, which GHL treats as "send via Custom Provider" — it records the message AND fires the deliveryUrl webhook to actually deliver. Our deliveryUrl `/api/integrations/outbound` then re-sent via Green API, so every outbound mirror caused the customer to receive a duplicate. Switched to `/conversations/messages/inbound` with `direction: "outbound"`, which records the historical outbound without firing the delivery callback. Added a 60-second text+contact dedup safety net in `/api/integrations/outbound` to defend against future regressions. Also fixes the "GHL only shows the message after a 2nd send" symptom, which was a side effect of the slow loop. ([integrations/ghl/client.ts](integrations/ghl/client.ts), [app/api/integrations/outbound/route.ts](app/api/integrations/outbound/route.ts))
