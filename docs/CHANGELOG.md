...
- **Calculator — custom quantity + reverse-margin check.** Two same-day additions:
  ...
  - "בדיקת רווח לפי מחיר" section with a "סכום כולל / מחיר ליחידה" toggle. Eli types a price; client-side reversal of `selling = (totalCost − shipping) × (1 + m/100) + shipping` returns the implied %, plus per-unit & total profit. Result chip greens out when the implied % meets/exceeds the configured margin.
