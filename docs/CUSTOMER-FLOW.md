...
### כלל מרכזי
**הבוט אוטונומי לחלוטין חוץ מ-DROPPED + רגעי money/escalation.**
- הבוט יכול לשנות את ה-state של ה-lead **לבד**, בכל שלב, ללא אישור אלי.
- הבוט יכול לסמן עסקה כ-**WON** לבד (אחרי הסכמה סופית בשלב 4).
- **חריג 1:** רק **אלי** יכול לסמן lead כ-DROPPED.
- **חריג 2 (v3.5 — Bot Supervisor Phase 1):** LLM supervisor רץ על כל הודעה לפני שיוצא. אם זה money moment (מחיר מתחרה, בקשת הנחה ספציפית, בקשת שיחה) → הבוט **לא שולח אוטומטית**. במקום: draft נכנס לתור אישור (`/dashboard/v3/drafts`) + DM לאלי. הלקוח רואה תשובה רק אחרי שאלי לוחץ approve.
