...
/**
 * Widget variant — POSTs to /api/widget/factory/[id]/send-to-feishu.
 */
...
export function HistoryDetailModalWidget({
  apiToken,
  row,
  onClose,
  onChanged,
        ...
        <div className="p-4 space-y-3">
          <section className="rounded-lg border border-border bg-background/40 p-3">
            ...
            <dl className="text-xs">
              ...
              {sizeHe && <DetailRow icon={<Package className="size-3" />} label="מידות" value={sizeHe} />}
              {materialHe && <DetailRow icon={<Package className="size-3" />} label="חומר" value={materialHe} />}
              <DetailRow icon={<Hash className="size-3" />} label="כמות" value={`${spec.quantity.toLocaleString("he-IL")} יח׳`} />
              {printingHe && <DetailRow icon={<Palette className="size-3" />} label="הדפסה" value={printingHe} />}
              {finishingHe && <DetailRow icon={<Package className="size-3" />} label="גימור" value={finishingHe} />}
              {spec.notes && <DetailRow icon={<Package className="size-3" />} label="הערות" value={spec.notes} />}
            </dl>
          </section>
          ...
          {resp && (
            <section className="rounded-lg border border-border bg-background/40 p-3">
              ...
              <dl className="text-xs">
                ...
                {resp.cartonCbm !== undefined && <DetailRow label="CBM" value={resp.cartonCbm.toFixed(3)} />}
                {resp.weightKg !== undefined && <DetailRow label="משקל" value={`${resp.weightKg} ק״ג`} />}
            ...
            <section className="rounded-lg border border-success/30 bg-success/5 p-3">
              ...
              <dl className="text-xs">
                ...
                {p.shippingOptionName && (
                  <DetailRow icon={<Truck className="size-3" />} label="שילוח" value={p.shippingOptionName} />
                )}
